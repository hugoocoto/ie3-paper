...
#let ieee(
    ...
    #grid(
      ...
      [#{
        for (i, e) in it.lines.enumerate() [
          #if code-lines { text(fill: luma(60%))[#(if i + 1 < 10 { " " + str(i + 1) } else { str(i + 1) })] }
          #h(1em)#e#linebreak()]
      }],
  ...
  set std.bibliography(title: text(10pt)[#references-text], style: "ieee")
  ...
  if (summary != none) {
    place(
      ...
      float: true,
      box(width: 80%)[
        #heading(level: 1, bookmarked: false, numbering: none)[#(summary-text)]
        #summary
      ],
    )
  }
  ...
  if abstract != none {
    ...
    [#(abstract-text)---#h(weak: true, 0pt)#abstract]

    if index-terms != () {
      parbreak()
      [#(index-terms-text)---#h(weak: true, 0pt)#index-terms.join[, ]]
    }
    v(2pt)
  }
